...
class Sidebar_Generator {
        ...
        <div class="widgets-holder-wrap">
            <div id="axil-new-sidebar" class="widgets-sortables">
                <div class="sidebar-name">
                    ...
                    <h2><?php esc_html_e( 'Add New Sidebar', 'axil-elements' ); ?><span class="spinner"></span></h2>
                </div>
                <div class="sidebar-description">
                    <form style="padding:0 7px;" method="POST" action="<?php echo esc_url( admin_url( 'admin-ajax.php?action=axil_add_sidebar' ) );?>">
                        ...
                        <table class="form-table">
                            <tr>
                                <th scope="row"><?php esc_html_e( 'Name', 'axil-elements' ) ?></th>
                                <td><input type="text" class="text" name="name" value=""></td>
                                <td><input type="submit" class="button-primary" value="<?php esc_html_e( 'Add New', 'axil-elements' ) ?>"></td>
                            </tr>
                        </table>
                    </form>
                </div>
    ...
    public function load_scripts() {
        ...
        $localize_data = array(
            'confirm'  => esc_html__( 'Are you sure you want to remove this custom sidebar', 'axil-elements' ),
            'failed'   => esc_html__( 'Operation failed' , 'axil-elements' ),
    ...
    public function ajax_add_sidebar() {
        ...
        if ( empty( $name ) ) {
            wp_send_json_error( esc_html__( "Sidebar name can't be empty", 'axil-elements' ) );
        }
        if ( empty( $nonce ) ) {
            wp_send_json_error( esc_html__( 'Empty nonce', 'axil-elements' ) );
        }
        if ( ! wp_verify_nonce( $nonce, 'axil_add_sidebar' ) ) {
            wp_send_json_error( esc_html__( 'Invalid nonce', 'axil-elements' ) );
        }
        ...
        if ( array_key_exists( $id, $sidebars ) ) {
            wp_send_json_error( esc_html__( 'Sidebar with the same name already exists. Please choose a different name', 'axil-elements' ) );
        }
    ...
    public function ajax_remove_sidebar() {
        ...
        if ( empty( $id ) ) {
            wp_send_json_error( esc_html__( 'Sidebar ID not found', 'axil-elements' ) );
        }
        if ( empty( $nonce ) ) {
            wp_send_json_error( esc_html__( 'Empty nonce', 'axil-elements' ) );
        }
